    ...
    def run_unix(self):
        if os.path.exists(self.build_lib):
            ...
            if not os.path.exists(dest_dir):
                try:
                    os.symlink(
                        os.path.abspath("third_party/gfootball_engine"), dest_dir
                    )
                except OSError as err:
                    raise err(
                        "Google Research Football: Could not create symlink to %s"
        ...
        try:
            ...
            raise ValueError(
                "Could not parse GFOOTBALL_USE_PREBUILT_SO environment "
        ...
        if use_prebuilt_lib:
            ...
            # Copy them all
            libs = glob.glob("third_party/gfootball_engine/_gameplayfootball*.so")
            copy_compiled_libs(libs, dest_dir)
        copy_fonts(dest_dir)

    def run_windows(self):
        guide_message = (
            "Please follow the guide on how to install prerequisites: "
        ...
        if not os.environ.get("VCPKG_ROOT"):
            raise OSError(
                "VCPKG_ROOT environment variable is not defined.\n" + guide_message
            )
        ...
        if os.path.exists(self.build_lib):
            ...
            if not os.path.exists(dest_dir):
                try:
                    os.symlink(
                        os.path.abspath("third_party/gfootball_engine"),
                        dest_dir,
...
setup(
    ...
    version="2.10.3",
    description=(
        "Google Research Football - RL environment based on "
        "open-source game Gameplay Football"
    ),
    long_description=(
        "Please see [our GitHub page](https://github.com/google-research/football) "
        "for details."
    ),
