...
flags.DEFINE_string(
    "level", "academy_empty_goal_close", "Defines type of problem being solved"
)
flags.DEFINE_enum(
    "state",
    "extracted_stacked",
    ["extracted", "extracted_stacked"],
...
flags.DEFINE_enum(
    "policy",
    "cnn",
    ["cnn", "lstm", "mlp", "impala_cnn", "gfootball_impala_cnn"],
    "Policy architecture",
)
...
flags.DEFINE_bool(
    "dump_full_episodes", False, "If True, trace is dumped after every episode."
)
flags.DEFINE_bool(
    "dump_scores", False, "If True, sampled traces after scoring are dumped."
)
...
def train(_):
    ...
    vec_env = SubprocVecEnv(
        [(lambda _i=i: create_single_football_env(_i)) for i in range(FLAGS.num_envs)],
        context=None,
    )
