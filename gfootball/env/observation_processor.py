...
class TextWriter(object):
    ...
    def write_arrow(self, arrow_type, scale_factor=1):
        ...
        (s_x, s_y, e_x, e_y) = tuple(
            int(v * scale_factor) for v in arrow_offsets[arrow_type]
        )
        start_point = (self._pos_x + s_x, self._pos_y + s_y)
        end_point = (self._pos_x + e_x, self._pos_y + e_y)
        image = cv2.arrowedLine(
            self._frame, start_point, end_point, self._color, thickness
        )
        ...
        if self._video_writer:
            ...
            if self._config["display_game_stats"]:
                ...
                if o["ball_owned_team"] == 0:
                    player = (
                        "G"
                        if o["left_team_roles"][o["ball_owned_player"]]
                        == e_PlayerRole_GK
                        else o["ball_owned_player"]
                    )
                    writer.write("BALL OWNED: %s" % player, color=(47, 68, 238))
                elif o["ball_owned_team"] == 1:
                    player = (
                        "G"
                        if o["right_team_roles"][o["ball_owned_player"]]
                        == e_PlayerRole_GK
                        else o["ball_owned_player"]
                    )
                ...
                for team in ["left", "right"]:
                    ...
                    for player in range(len(o[sticky_actions_field])):
                        assert len(sticky_actions) == len(
                            o[sticky_actions_field][player]
                        )
                        ...
                        for i in range(len(sticky_actions)):
                            if sticky_actions[i]._directional:
                                ...
                                player_info[sticky_actions[i]._name] = o[
                                    sticky_actions_field
                                ][player][i]
                        ...
                        if "action" in o._trace["debug"]:
                            # Info about action
                            player_info["ACTION"] = o["action"][len(players_info)]._name
...
class ObservationProcessor(object):
    ...
    def write_dump(self, name):
        ...
        config._active_dump = ActiveDump(
            dump_name, self._frame + config._steps_after, self._config
        )
