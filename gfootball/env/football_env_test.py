...
class FootballEnvTest(parameterized.TestCase):
    ...
    def test___render(self):
        ...
        if platform.system() == "Windows":
            ...
            if sys.maxsize > 2**32:  # x64
                expected_hash_value = (
                    683941870 if hash_value != 3676773624 else 3676773624
                )
            else:  # x86
                expected_hash_value = (
                    1490754124 if hash_value != 2808421794 else 2808421794
                )
    ...
    def test_player_order_invariant(self):
        ...
        cfg = config.Config(
            {"level": "tests.11_vs_11_hard_deterministic", "players": players}
        )
        env = football_env.FootballEnv(cfg)
        actions = len(football_action_set.get_action_set(cfg))
    ...
    def test_symmetry(self, seed):
        ...
        actions = [
            random.randint(0, action_cnt - 1) for _ in range(10 if fast_run else 3000)
        ]
        queue1 = Queue()
        thread1 = threading.Thread(target=run_scenario, args=(cfg1, queue1, actions))
        thread1.start()
        queue2 = Queue()
        thread2 = threading.Thread(target=run_scenario, args=(cfg2, queue2, actions))
        thread2.start()
        while True:
            o1 = queue1.get()
            o2 = queue2.get()
    ...
    @parameterized.parameters(
        (1, "left", True), (0, "right", True), (1, "left", False), (0, "right", False)
    )
    ...
    @parameterized.parameters(
        (0, 1, True), (1, -1, True), (0, 1, False), (1, -1, False)
    )
    def test_corner(self, episode, factor, reverse):
        ...
        while not done:
            o, _, done, _ = env.step(
                [football_action_set.action_left, football_action_set.action_left]
            )
    ...
    def test_get_state_wrapper(self):
        env = gfootball.env.create_environment(
            stacked=True, env_name="academy_empty_goal", rewards="checkpoints,scoring"
        )
        o = env.reset()
        state = env.get_state()
        reward1 = 0
